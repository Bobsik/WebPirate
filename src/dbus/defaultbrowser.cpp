...
const QString DefaultBrowser::OPEN_URL = "harbour-webpirate-open-url.desktop";
...
void DefaultBrowser::setEnabled(bool b)
{
 ...
 //       this->restoreMime();
    ...
    proc->start("update-desktop-database "+ h +"/.local/share/applications"); // + localappssdir);
    emit enabledChanged();
}
...
bool DefaultBrowser::isMimeOverriden()
{
    ...
    if(res && this->executeXdgMime("query default x-scheme-handler/http") != DefaultBrowser::OPEN_URL)
        res = false;

    if(res && this->executeXdgMime("query default x-scheme-handler/https") != DefaultBrowser::OPEN_URL)
        res = false;
...
void DefaultBrowser::writeLocalOpenUrl() const
{
    ...
    f.write(QString("[Desktop Entry]\n"
                    "Type=Application\n"
                    "Name=Open with WebPirate\n"
                    "Icon=harbour-webpirate\n"
                    "NoDisplay=true\n"
...
void DefaultBrowser::overwriteMime()
{
    ...
    this->setMime("x-scheme-handler/http", DefaultBrowser::OPEN_URL);
    this->setMime("x-scheme-handler/https", DefaultBrowser::OPEN_URL);
...
void DefaultBrowser::restoreMime()
{
    ...
    this->setMime("x-scheme-handler/http", "open-url.desktop");
    this->setMime("x-scheme-handler/https", "open-url.desktop");
...
{
    this->_enabled =this->localServiceExists() && this->localOpenUrlExists();// this->isMimeOverriden() &&
    emit enabledChanged();
}
